...
<div class="container my-5" id="connection">
    <div class="row mb-4">
        ...
        <div class="col-sm-6 order-1 order-md-2">  
            <div class="text-center mx-auto">
                <h2 class="text-center">Businesses That Need You</h2>
            </div>
        </div>
            ...
            <div class="dropdown-menu bg-white dropdown-menu-right navbar-dropdown preview-list mainNavbarDropdownMenu" aria-labelledby="messageDropdown" style="border: none;">
                @if(count($dropdown) >=1)
                    ...
                    @foreach($dropdown as $key=>  $data) 
                        <a href="javascript:void(0)" class="btn btn-outline-success dropdown-item filterByDropdown" rel="{{$key}}" style="font-size:14px">{{$data}}</a>   
                    @endforeach  
                @endif   
            </div>
        </div>
    ...
    <div class="row" id="findFilteredMatches">
        ...
        <div class="col-md-3  mb-md-5 mb-lg-0 d-flex align-items-stretch usersMatchesDetails {{$ids}} @if($data['connection'] > 1) matched @else pending @endif">
            <div class="card card-profile mt-4 proivdeMatchesUser">  
                        ...
                        <div class="pt-4 ">
                            ...
                            <div>
                                ...
                                <div> <span style="color:#000">{{$key}}</span>
                                    <div class="owl-carousel-provide owl-carousel">
                                        @foreach($c as $k)
...
<script>
    var owlss = $('.owl-carousel-provide');
    owlss.owlCarousel({
        autoplay: false,
        center: true,
    ...
    $(document).on('click', '.getCategoryId', function () {
        ...
        $.ajax({
            ...
            type: "POST",
            url: "/moreProvideMatchesSession",
            data: {cat: cat, subCat: subCat},
            // dataType: 'html',
            success: function (data)
            {
                window.location.href = '/more-provide-matches';
            }
        });
    });
