            ...
            <div class="col-lg-6 col-md-8 col-sm-10 col-12 mx-auto">
                <div class="card surveyBoxSize surveyBox-5  scale-up-center mt-2"> 
                    ...
                    <div class="card-body" style="color:#333;" id="page_7">
                        <form method="post" action="{{route('submitEditedSurvey6')}}" class="surveyForms">
                            @csrf
                            @php $c= 1; @endphp
                            @if(!empty($val['question']))
                            @php $count=$val['question']; @endphp
                            @foreach($val['question'] as $key=> $qu)                            
                            <div class="questionDiv " @if($key != 0)style="display:none;"@endif>
                                 <div class="progress mb-3" >
                                    <div class="progress-bar progress-bar-striped progress-bar-animated bg-warning" role="progressbar"  aria-valuemin="0" 
                                         aria-valuemax="100" style="width: <?php echo (100 / count($count)) * ($c); ?>%"><?php echo 'Question ' . $c++; ?></div>
                                </div>
                                ...
                                <div class="@if($key != 0) scale-up-hor-right @endif ">
                                    <label class="form-check-label suveyLabel"  for="defaultCheck1">{{$qu}}</label>
                                    <input type="hidden" name="question[]" value="{{$qu}}">
                                    ...
                                    <div class="mt-4">
                                        ...
                                        @endif
                                        <button type="button" class="btn btn-sm btn-success float-right nextQuestion" style="padding: 5px 11px;" >Next</button>
                                    </div> 
                                </div>
                            </div>
                            @endforeach
                            ...
                            <div class="questionDiv " @if($key != 0)style="display:none;"@endif>
                                 <div class="progress mb-3" >
                                    <div class="progress-bar progress-bar-striped progress-bar-animated bg-warning" role="progressbar"  aria-valuemin="0" 
                                         aria-valuemax="100" style="width: <?php echo (100 / count($list)) * ($c); ?>%"><?php echo 'Question-' . $c++; ?></div>
                                </div>
                                <p style="font-weight:600;font-size: 17px;" class="hideForLast">Please fill out the following questions to submit your application.</p>
                                <div class="@if($key != 0) scale-up-hor-right @endif ">
                            ...
                            <div class="questionDiv scale-up-hor-right" style="display:none;">
                                <div class="d-flex">
                                    ...
                                    <label for="survey7" style="font-size:10px;">ConnectEO Network is a space where everyone wins. As a member of the community, you agree to respect this space, add value and treat all of the entrepreneurs with honor and respect. You understand that there is no tolerance for bullying, harassment or toxic behavior. A violation of the above will result in removal from the community.</label>
                                </div>
